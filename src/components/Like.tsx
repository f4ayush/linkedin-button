...
function Like() {
    ...
    const [reaction, setReaction] = useState<{ image: StaticImageData; text: string }>()
                ...
                <div className="reactions">
                    {
                        imageData.map((data, index) => (
                            <Image key={index} src={data.image} className='icons' alt={data.text} onClick={()=>handleReactionSelection(index)} height={25}/>
                        ))
                    }
                </div>
            }
